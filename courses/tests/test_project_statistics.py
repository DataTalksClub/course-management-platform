...
class ProjectStatisticsTestCase(TestCase):
    ...
    def test_calculate_raw_project_statistics_basic(self):
        ...
        for i, scores in enumerate(submissions_data):
            self.create_project_submission(self.users[i], self.enrollments[i], scores)
        ...
        self.assertEqual(stats['total_score']['avg'], 17.666666666666668)
    ...
    def test_calculate_raw_project_statistics_insufficient_data(self):
        ...
        for i in range(2):
            self.create_project_submission(self.users[i], self.enrollments[i])
    ...
    def test_calculate_project_statistics_model_creation(self):
        ...
        for i in range(3):
            scores = {'project_score': 10 + i, 'total_score': 20 + i, 'time_spent': 10.0 + i}
            self.create_project_submission(self.users[i], self.enrollments[i], scores)
        
        # Should not exist initially
        self.assertFalse(ProjectStatistics.objects.filter(project=self.project).exists())
        
        # Calculate statistics
        stats = calculate_project_statistics(self.project)
        
        # Should create the object
        self.assertTrue(ProjectStatistics.objects.filter(project=self.project).exists())
    ...
    def test_calculate_project_statistics_force_update(self):
        ...
        self.create_project_submission(self.users[3], self.enrollments[3])
    ...
    def test_calculate_project_statistics_uncompleted_project(self):
        ...
        self.assertIn("Cannot calculate statistics for uncompleted project", str(context.exception))
    ...
    def test_project_statistics_model_methods(self):
        ...
        self.assertEqual(len(first_field), 3)  # (name, stats_list, icon)
...
class ProjectStatisticsViewTestCase(TestCase):
    ...
    def test_project_statistics_view_success(self):
        ...
        with patch('courses.views.project.calculate_project_statistics') as mock_calc:
            ...
            url = reverse('project_statistics', args=[self.course.slug, self.project.slug])
    ...
    def test_project_statistics_view_incomplete_project(self):
        ...
        url = reverse('project_statistics', args=[self.course.slug, self.incomplete_project.slug])
        response = self.client.get(url, follow=True)
        
        # Should redirect to project page
        self.assertRedirects(
            response,
            reverse('project', args=[self.course.slug, self.incomplete_project.slug])
        )
        
        # Should show error message
        messages = list(response.context['messages'])
        self.assertTrue(any("not completed yet" in str(m) for m in messages))
    
    def test_project_statistics_view_nonexistent_project(self):
        """Test project statistics view with non-existent project"""
        url = reverse('project_statistics', args=[self.course.slug, 'nonexistent'])
        response = self.client.get(url)
    ...
    def test_project_statistics_view_nonexistent_course(self):
        ...
        url = reverse('project_statistics', args=['nonexistent', self.project.slug])
        response = self.client.get(url)
...
class ProjectStatisticsIntegrationTestCase(TestCase):
    def setUp(self):
        ...
        # Create diverse submissions for comprehensive statistics
        self.users = []
        self.enrollments = []
        for i in range(10):
            user = User.objects.create_user(
    ...
    def test_full_statistics_workflow(self):
        ...
        submission_data = [
            {'project_score': 5, 'project_learning_in_public_score': 2, 'peer_review_score': 1, 'peer_review_learning_in_public_score': 0, 'total_score': 8, 'time_spent': 5.0},
            {'project_score': 8, 'project_learning_in_public_score': 4, 'peer_review_score': 2, 'peer_review_learning_in_public_score': 1, 'total_score': 15, 'time_spent': 8.0},
            {'project_score': 12, 'project_learning_in_public_score': 6, 'peer_review_score': 3, 'peer_review_learning_in_public_score': 2, 'total_score': 23, 'time_spent': 12.0},
            {'project_score': 10, 'project_learning_in_public_score': 5, 'peer_review_score': 3, 'peer_review_learning_in_public_score': 1, 'total_score': 19, 'time_spent': 10.0},
            {'project_score': 15, 'project_learning_in_public_score': 8, 'peer_review_score': 3, 'peer_review_learning_in_public_score': 2, 'total_score': 28, 'time_spent': 15.0},
        ]
        ...
        project_url = reverse('project', args=[self.course.slug, self.project.slug])
    ...
    def test_statistics_links_in_navigation(self):
        ...
        stats_url = reverse('project_statistics', args=[self.course.slug, self.project.slug])
        
        # Test project results page has statistics link
        results_url = reverse('project_results', args=[self.course.slug, self.project.slug])
        ...
        list_url = reverse('project_list', args=[self.course.slug, self.project.slug])
    ...
    def test_statistics_links_only_for_completed_projects(self):
        ...
        stats_url = reverse('project_statistics', args=[self.course.slug, incomplete_project.slug])
        
        # Test project page doesn't show statistics link for incomplete project
        project_url = reverse('project', args=[self.course.slug, incomplete_project.slug])
        project_response = self.client.get(project_url)
        ...
        list_url = reverse('project_list', args=[self.course.slug, incomplete_project.slug])
        list_response = self.client.get(list_url)
...
class ProjectStatisticsAdminTestCase(TestCase):
    ...
    def test_calculate_statistics_admin_action_success(self):
        ...
        with patch('courses.admin.projects.calculate_project_statistics') as mock_calc:
            ...
            mock_calc.assert_called_once_with(self.completed_project, force=True)
    ...
    def test_calculate_statistics_admin_action_mixed_projects(self):
        ...
        queryset = Project.objects.filter(
            id__in=[self.completed_project.id, self.incomplete_project.id]
        )
        request = Mock()
        request.user = self.admin_user
    ...
    def test_admin_action_is_registered(self):
        ...
        self.assertIn('calculate_statistics_selected_projects', action_names)
        
        # Test action description
        action_func, name, description = actions['calculate_statistics_selected_projects']
