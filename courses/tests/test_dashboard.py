...
class DashboardViewTestCase(TestCase):
    ...
    def test_dashboard_context_basic(self):
        ...
        self.assertEqual(response.context['total_enrollments'], 6)  # 1 main user + 5 additional
...
class DashboardHomeworkStatsTestCase(TestCase):
    ...
    def test_homework_statistics_calculation(self):
        ...
        submission_data = [
            {'time_spent_lectures': 1.0, 'time_spent_homework': 2.0, 'total_score': 80},
            {'time_spent_lectures': 2.0, 'time_spent_homework': 3.0, 'total_score': 85},
            {'time_spent_lectures': 3.0, 'time_spent_homework': 4.0, 'total_score': 90},
            {'time_spent_lectures': 4.0, 'time_spent_homework': 5.0, 'total_score': 95},
            {'time_spent_lectures': 5.0, 'time_spent_homework': 6.0, 'total_score': 100},
        ]
        
        for i, data in enumerate(submission_data):
            self.create_homework_submission(self.users[i], self.enrollments[i], data)
    ...
    def test_homework_statistics_with_insufficient_data(self):
        ...
        for i in range(2):
            self.create_homework_submission(self.users[i], self.enrollments[i])
    ...
    def test_homework_statistics_with_null_values(self):
        ...
        submission_data = [
            {'time_spent_lectures': 2.0, 'time_spent_homework': 3.0, 'total_score': 85},
            {'time_spent_lectures': None, 'time_spent_homework': 4.0, 'total_score': 90},
            {'time_spent_lectures': 3.0, 'time_spent_homework': None, 'total_score': 80},
            {'time_spent_lectures': 4.0, 'time_spent_homework': 5.0, 'total_score': 95},
        ]
    ...
    def test_homework_formatted_time_display(self):
        ...
        self.create_homework_submission(
            self.users[0], 
            self.enrollments[0], 
            {'time_spent_lectures': 3.0, 'time_spent_homework': 4.0, 'total_score': 85}
        )
        ...
        for i in range(1, 4):
            self.create_homework_submission(
                self.users[i], 
                self.enrollments[i], 
                {'time_spent_lectures': 3.0 + i, 'time_spent_homework': 4.0 + i, 'total_score': 85 + i}
            )
...
class DashboardProjectStatsTestCase(TestCase):
    def setUp(self):
        ...
        # Create users for project submissions
        self.users = []
        self.enrollments = []
        for i in range(6):
            user_creds = {
    ...
    def create_project_submission(self, user, enrollment, scores=None, passed=True):
    ...
    def test_project_statistics_calculation(self):
        ...
        for i, data in enumerate(submission_data):
            passed = data.pop('passed')
            self.create_project_submission(self.users[i], self.enrollments[i], data, passed)
        ...
        self.assertAlmostEqual(response.context['project_completion_rate'], expected_completion, places=1)
    ...
    def test_project_statistics_with_no_submissions(self):
        ...
        self.assertEqual(response.context['project_completion_rate'], 0.0)
    ...
    def test_avg_total_score_calculation(self):
        ...
        self.assertAlmostEqual(response.context['avg_total_score'], expected_avg, places=1)
...
class DashboardIntegrationTestCase(TestCase):
    ...
    def test_dashboard_with_complete_course_data(self):
        ...
        for i, (user, enrollment) in enumerate(zip(users[:3], enrollments[:3])):
        ...
        hw1_stats = next(stat for stat in hw_stats if stat['homework'] == self.homework1)
        hw2_stats = next(stat for stat in hw_stats if stat['homework'] == self.homework2)
