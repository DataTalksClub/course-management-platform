...
def calculate_wrapped_statistics(year=2025, force=False):
    ...
    stats, created = WrappedStatistics.objects.get_or_create(
        year=year
    )
    
    if not force and not created:
        logger.info(f"Wrapped statistics for {year} already exist. Use force=True to recalculate.")
        return stats
    ...
    ).select_related('homework', 'homework__course', 'enrollment', 'student')
    
    # Get all project submissions in the year
    project_submissions_2025 = ProjectSubmission.objects.filter(
        submitted_at__gte=year_start,
        submitted_at__lte=year_end
    ).select_related('project', 'project__course', 'enrollment', 'student')
    
    # Get unique students with activity in 2025
    students_from_homeworks = set(hw.student for hw in homework_submissions_2025)
    students_from_projects = set(proj.student for proj in project_submissions_2025)
    students_with_2025_activity = students_from_homeworks | students_from_projects
    
    logger.info(f"Found {len(students_with_2025_activity)} students with activity in {year}")
    ...
    stats.total_points = enrollments_for_active_students.aggregate(
    ...
    for course in courses_with_2025_activity:
        enrollment_count = enrollments_for_active_students.filter(course=course).count()
    ...
    course_stats_list.sort(key=lambda x: x['enrollment_count'], reverse=True)
    ...
    for enrollment in enrollments_for_active_students:
        ...
        user_scores[enrollment.student_id]['total_score'] += enrollment.total_score or 0
    ...
    logger.info(f"Platform statistics calculated. Now calculating individual user statistics...")
    ...
    for student in students_with_2025_activity:
        ...
        user_homework_hours = sum(
            (hw.time_spent_lectures or 0) + (hw.time_spent_homework or 0)
            for hw in user_homework_submissions
        )
        user_project_hours = sum(
            proj.time_spent or 0
        ...
        faq_homework = sum(
            1 for hw in user_homework_submissions
            if hw.faq_contribution and hw.faq_contribution.strip()
        )
        faq_projects = sum(
            1 for proj in user_project_submissions
        ...
        # Get display name
        display_name = user_enrollments[0].display_name if user_enrollments else student.username
